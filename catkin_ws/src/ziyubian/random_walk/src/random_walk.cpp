	...
	void spin() {
		...
		while(ros::ok()) { // Keep spinning loop until user presses Ctrl+C	
			...
			// depending on the FSM state; also change the FSM state when appropriate  
      			/////////////////////// ANSWER CODE BEGIN ///////////////////
      		//ROS_INFO_STREAM("Time: " << rotateStartTime);
      		ros::Time rotateStartTime(ros::Time::now());
      		double distance = 10;
      		float a;
      		ROS_INFO_STREAM("fsm: " << fsm);
      		a = distance/FORWARD_SPEED_MPS;
      		ros::Duration rotateDuration(a);
      		if(fsm == FSM_MOVE_FORWARD && ros::Time::now()>=rotateStartTime+rotateDuration){
