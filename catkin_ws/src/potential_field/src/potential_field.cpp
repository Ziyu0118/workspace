...
public:
  ...
  void laserCallback(const sensor_msgs::LaserScan::ConstPtr& msg) {
    ...
    float epsilon = 0.05;
    float dsafe = 2.5;
    double MIN_SCAN_ANGLE_RAD = -10.0/180*M_PI;
    double MAX_SCAN_ANGLE_RAD = +10.0/180*M_PI;
  ...
  void spin() {
    ...
    while (ros::ok()) { // Keep spinning loop until user presses Ctrl+C
      ...
      //ROS_INFO_STREAM("ROBOT_heading: " << pose[i].heading);
      
        
      
      //ROS_INFO_STREAM("theta_diff: " << theta_diff);
      //ROS_INFO_STREAM("theta_diff: " << atan2(force_y,force_x) - heading - M_PI/2);
      /*if (abs(force_y) <= 0.5 && abs(force_x) <= 0.5 && ros::Time::now()>=annealStartTime+annealDuration){
          force_x = 4 + rand()%10;
          force_y = 4 + rand()%10;
          theta_diff = atan2(force_y, force_x);
          ROS_INFO_STREAM("Annealing!");
      }*/
